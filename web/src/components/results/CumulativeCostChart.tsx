...
export default function CumulativeCostChart({ monthly }: Props) {
  const data = monthly.map((m, i) => ({ month: i, buy: Math.round(m.cumulative_buy_cost), rent: Math.round(m.cumulative_rent_cost) }));
  return (
    <ChartCard title="Cumulative Housing Cost">
      <ResponsiveContainer width="100%" height={280}>
        <LineChart data={data}>
          ...
          <Line type="monotone" dataKey="buy" stroke="#6366f1" strokeWidth={2.5} dot={false} name="Buying" />
          <Line type="monotone" dataKey="rent" stroke="#f59e0b" strokeWidth={2.5} dot={false} name="Renting" />
        </LineChart>
      </ResponsiveContainer>
    </ChartCard>
  );
